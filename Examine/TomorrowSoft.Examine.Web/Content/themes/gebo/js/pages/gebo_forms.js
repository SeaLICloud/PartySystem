/* [ ---- Gebo Admin Panel - extended form elements ---- ] */
	...
	gebo_datepicker = {
		init: function() {
			...
			$dp_end.datepicker({format: "mm/dd/yyyy"}).on('changeDate', function(){
				var dateText = $(this).data('date');
	...
	gebo_sliders = {
	    init: function () {
	        var $ui_slider1 = $(".ui_slider1"),
				$ui_slider2 = $(".ui_slider2"),
				$ui_slider3 = $("#ui_slider3_sel");
	        ...
	        //* slider with select
//	        var slider = $("<div id='ui_slider3'></div>").insertAfter($ui_slider3).slider({
//	            min: 1,
//	            max: 6,
//	            range: "min",
//	            value: $ui_slider3[0].selectedIndex + 1,
//	            slide: function (event, ui) {
//	                $ui_slider3[0].selectedIndex = ui.value - 1;
//	            }
//	        });
//	        $ui_slider3.change(function () {
...
//	        });
	...
	gebo_multiselect = {
		init: function(){
			var $public_methods = $('#public-methods');

			if($public_methods.length) {
                //* public methods
                $public_methods.multiSelect();

                $('#select-all').click(function(){
                    $public_methods.multiSelect('select_all');
                    return false;
                });
                $('#deselect-all').click(function(){
                    $public_methods.multiSelect('deselect_all');
                    return false;
                });
                $('#select-fr').click(function(){
                    $public_methods.multiSelect('select', 'fr');
                    return false;
                });
                $('#deselect-fr').click(function(){
                    $public_methods.multiSelect('deselect', 'fr');
                    return false;
                });
            }
            if($('#optgroup').length) {
            ...
            if($('#searchable').length) {
                ...
                $('#searchable').multiSelect({
                    selectableHeader: '<div class="search-header"><input type="text" class="form-control" id="ms-search" autocomplete="off" placeholder="country name"></div>',
	...
	gebo_pass_check = {
		init: function() {
			$("#pass_check").complexify({
				...
				if (!valid) {
					$('#pass_progress').children('.progress-bar').css({'width':complexity + '%'}).removeClass('progress-bar-success').addClass('progress-bar-danger');
				} else {
					$('#pass_progress').children('.progress-bar').css({'width':complexity + '%'}).removeClass('progress-bar-danger').addClass('progress-bar-success');
				}
			});
		}
	};
